...
- 离线单测 **348 passed**（31 个 test_*.py + conftest，零外部依赖）；挑战集证据支持 `python -m eval challenge --rejudge` 零成本重判（不重跑 LLM），人工复核存 sidecar（训练结果数据/challenge_v2_review.json）；竞赛原始数据与 golden 基准本地保留、不入库。
...
- 测试健康：348 passed、0 失败 0 跳过；源码 TODO / FIXME / HACK 为 0 处；prompts 模块级版本号已补齐（B-13 起，registry.json 为唯一事实源）。
- 挑战集复跑新发现的缺陷（2026-09-10）与处置：B-31 已闭环——C2016「每股公积金」的近似字段替换（net_asset_per_share 答 24.00 元）与 C2007 股价/总市值回落技术性 SQL 报错，均由「库外指标统一出口」修复（契约扩为五键 unsupported_metrics，tools/native_financial.py:unsupported_metrics_of + prompts/fallback.py:build_refuse_metric_out_of_scope）；复跑 auto 15/15 pass、0 fail、3 pending（仅 binding 恒 pending）。B-30（指标-问题一致性校验）已于同日闭环——生成侧确定性校验 tool `metric_field_consistency_error`（SELECT 指标列 ⊆ standard_fields，函数/表达式/`*` 跳过不判）+ refuse.metric_mismatch 出口 + 11 条单测，真实 7 题形态两轮复验零误伤，另加库外指标漏标词表兜底（_OUT_OF_SCOPE_QUESTION_TERMS）；C2018 研报召回不稳定（3 次运行仅 1 次召回预测/评级段落）→ 归入 B-24 检索质量评估证据。
...
- 改动财务链路时除既有回归集外，另看 output_contract_stats.jsonl 的 kind=metric_consistency 命中率（越界即误伤或近似替换）；下一优先项：B-24 检索质量评估（补 C2018 召回证据）与 B-25 LLM-judge 校准。
