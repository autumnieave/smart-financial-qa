...
> 生成时间：2026-09-09（项目现状深度扫描）；**更新：2026-09-10（B-13~B-31 落地后口径刷新）**；证据来源见各小节引用的文件与行号。
...
- 近 10 次提交主线：B-13 prompts 版本注册表 → B-14 三层结构 → B-15/16 few-shot 与动态检索 → B-17 输出契约 → B-18 兜底话术 → B-19/20 API 契约与覆盖率门禁 → B-22 对抗挑战集 v2（18 条/5 类，真实执行 + 人工抽审）→ B-26~B-29 挑战集暴露缺陷修复（supervisor 误拒答 / 注入先拒答 / 错别字归一化 / 研报预测荐股转述口径）→ B-31 库外指标统一出口（unsupported_metrics + refuse.metric_out_of_scope，C2007/C2016/C2017 闭环）。
- 离线单测 **337 passed**（30 个 test_*.py + conftest，零外部依赖）；挑战集证据支持 `python -m eval challenge --rejudge` 零成本重判（不重跑 LLM），人工复核存 sidecar（训练结果数据/challenge_v2_review.json）；竞赛原始数据与 golden 基准本地保留、不入库。
...
- prompts 版本注册表（prompts/registry.json，7 条登记：6 业务模块 + 包级；当前包级 2026-09-10-v5、multi_agent v3、fallback v3、financial v9）+ 防漂移单测（tests/test_prompt_registry.py）。
...
- 测试健康：337 passed、0 失败 0 跳过；源码 TODO / FIXME / HACK 为 0 处；prompts 模块级版本号已补齐（B-13 起，registry.json 为唯一事实源）。
- 挑战集复跑新发现的缺陷（2026-09-10）与处置：B-31 已闭环——C2016「每股公积金」的近似字段替换（net_asset_per_share 答 24.00 元）与 C2007 股价/总市值回落技术性 SQL 报错，均由「库外指标统一出口」修复（契约扩为五键 unsupported_metrics，tools/native_financial.py:unsupported_metrics_of + prompts/fallback.py:build_refuse_metric_out_of_scope）；复跑 auto 15/15 pass、0 fail、3 pending（仅 binding 恒 pending）。B-30（聚合层「指标-问题一致性」校验）与 B-31 高度重叠，近似替换的生成侧入口已被封堵，剩余聚合层校验待做；C2018 研报召回不稳定（3 次运行仅 1 次召回预测/评级段落）→ 归入 B-24 检索质量评估证据。
...
- 挑战集 pending 项（仅剩绑定 3 条，恒 pending）在 B-25 LLM-judge 与人工双回查对齐 ≥90% 后启用自动判定（§6.7.3）。
- 优先修复 B-30 剩余部分（aggregator 层「指标-问题一致性」校验；生成侧近似替换已由 B-31 封堵），并按设计方案 §6.7.2 推进 B-24 检索质量评估（补 C2018 召回证据）。
