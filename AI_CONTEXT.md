...
- 测试健康：416 passed、0 失败 0 跳过；源码 TODO / FIXME / HACK 为 0 处；prompts 模块级版本号已补齐（B-13 起，registry.json 为唯一事实源）。
...
- **B-36（P0，未修）**：B2053 类**系统性误拒答**——同题 n=5 中 4 次回答「未查询到…（SQL 已执行成功，但未返回任何数据）」、第 1 次却给出 14 家完整答案 + 596 字 SQL，与 B-33 同源。检测盲区：judge 与规则信号**都判 pass**（二者均无「该题本应有数据」先验，后者拒答词表补 `未查询到/尚未收录/暂未收录/未返回任何数据` 后只会更倾向 pass）。修法 = 把 B2053 首跑答案 + SQL 固化为参考解法，`eval/llm_judge.py` 增加 `--answer-key`「应有数据」先验校验，再复跑 n=5 验证误拒答率 4/5 → 0/5；反向验证 B2004（合理拒答）与 B2036（B-30 守卫拒答）不得被新判据误伤。
